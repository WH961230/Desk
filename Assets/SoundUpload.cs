...
public class SoundUploader : MonoBehaviour {
    ...
    void Start() {
        ...
        viewButton.onClick.AddListener(ShowFileNames);
    ...
    IEnumerator HandleFileUpload(string sourcePath) {
        ...
        try {
            ...
            outputText.text = $"上传成功：{fileName}";

            // 验证加载
            StartCoroutine(LoadAudioFile(targetPath));
        } catch (System.Exception e) {
            outputText.text = $"上传失败：{e.Message}";
            yield break;
        }
    }
    ...
    private void ShowFileNames() {
        string[] files = Directory.GetFiles(soundFolderPath)
            .Where(p => IsValidAudioFile(p))
            .Select(p => Path.GetFileName(p))
            .ToArray();

        outputText.text = files.Length > 0 ? "已存储文件：\n" + string.Join("\n", files) : "暂无存储文件";
    }
